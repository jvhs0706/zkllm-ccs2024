...
G1TensorJacobian Commitment::commit(const FrTensor& t, bool speedup = true) const
{
    if (t.size % size != 0) throw std::runtime_error("Commitment::commit - Incompatible dimensions");

    uint m = t.size / size;
    G1TensorJacobian temp = (*this) * t;
    if (speedup) return temp.rowwise_sum(m, size);
    else {
        G1TensorJacobian out(m);
        com_sum_row_kernel<<<(m+G1NumThread-1)/G1NumThread,G1NumThread>>>(temp.gpu_data, out.gpu_data, m, size);
